...
const lbw = async (matchData, ballData, inning) => {
  const batsman = {
  ...
  let dataToUpdate = {
    ...
    "now.runRate": matchData.now.runs / ((matchData.now.overs * 6 + (matchData.now.balls + 1)) / 6),
  ...
  if (updateMatch.now.wickets === 10 || (updateMatch.superOver && updateMatch.now.wickets === 1)) {
