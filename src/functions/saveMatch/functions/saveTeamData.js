...
const saveTeamData = (matchData, docRef) => {
  ...
  if (matchData.type === "Group") {
    firestore
      ...
      .then((doc) => {
        ...
        const netRunRateA = {
          against: {
            runs: matchData.innings.second.runs + oldPointA.netRunRate.against.runs,
        ...
        const nrrA =
          netRunRateA.for.runs / (netRunRateA.for.overs + netRunRateA.for.balls / 6) -
          netRunRateA.against.runs / (netRunRateA.against.overs + netRunRateA.against.balls / 6);

        const netRunRateB = {
          against: {
            runs: matchData.innings.first.runs + oldPointB.netRunRate.against.runs,
        ...
        const nrrB =
          netRunRateB.for.runs / (netRunRateB.for.overs + netRunRateB.for.balls / 6) -
          netRunRateB.against.runs / (netRunRateB.against.overs + netRunRateB.against.balls / 6);
        ...
        firestore
          ...
          .doc(matchData.tournament.key)
          .update({
            pointTable: firebase.firestore.FieldValue.arrayUnion(pointA, pointB),
          })
          .then(() => {
            firestore
              .collection("leagues")
              ...
              .doc(matchData.tournament.key)
              .update({
                pointTable: firebase.firestore.FieldValue.arrayRemove(oldPointA, oldPointB),
              });
          });
      });
  }
  ...
  firestore
    ...
    .then((doc) => {
      ...
      const netRunRateA = {
        against: {
          runs: matchData.innings.second.runs + oldTeamA.netRunRate.against.runs,
      ...
      firestore
        ...
        .update({
          ...
          points: teamAMatches.points + (teamAMatches.won / teamAMatches.played) * 1000 + nrrA * 10,
        });
    });
    ...
    .then((doc) => {
      ...
      firestore
        ...
        .update({
          ...
          points: teamBMatches.points + (teamBMatches.won / teamBMatches.played) * 1000 + nrrB * 10,
        });
    });
};
